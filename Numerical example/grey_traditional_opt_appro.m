...
%%
% load data
load traditional_method_estimates.mat
% optimazation
T_interval=[1 10];
...
% title({'(a) 近似成本'},'FontSize',16)
set(gca,'FontName','Microsoft YaHei','FontSize',14)
nexttile
fplot(cost_der,T_interval,'LineWidth',1.5)
xlabel({'订货周期/日'},'FontSize',14)
ylabel(['近似成本的一阶导数'],'FontSize',14)
% title({'(a) 近似成本的一阶导数'},'FontSize',16)
set(gca,'FontName','Microsoft YaHei','FontSize',14)
nexttile
fplot(cost_der2,T_interval,'LineWidth',1.5)
xlabel({'订货周期/日'},'FontSize',14)
ylabel(['近似成本的二阶导数'],'FontSize',14)
% title({'(a) 近似成本的二阶导数'},'FontSize',16)
set(gca,'FontName','Microsoft YaHei','FontSize',14)
%%
Q_opt=d/theta*(exp(theta*T_opt)-1);
% save figure
savefig(gcf,'.\figure\grey_traditional_opt_appro.fig');
exportgraphics(gcf,'.\figure\grey_traditional_opt_appro.pdf')
